...
export default function FindPeople(props) {
  ...
  useEffect(() => {
    ...
    Axios({
      method: 'GET',
      url: `http://localhost:5000/profiles/FindPeople/${
        searchPeople || 'd9r3'
      }`,
      withCredentials: true,
    }).then((res) => {
  ...
  return (
    <React.Fragment>
          ...
          return (
            <div key={person._id}>
              <a
                ...
                name={person._id}
                target="_blank"
                otheridtoparents={setOtherProfileID}
                onClick={sendPropsParents}
              >
      ...
      {errors && (
        <div>
          {' '}
          Woops, there was an error loading your search, please try again!
        </div>
      )}
    </React.Fragment>
  );
